...
class TestHiccdStitchCube(unittest.TestCase):
    ...
    def test_set_cubenorm_lines(self, mock_getkey):
        ...
        self.assertRaises(ValueError,
                          c.set_cubenorm_lines, 1, 1, 2000, None, 2)
        self.assertRaises(ValueError,
                          c.set_cubenorm_lines, 1, 1, None, 2000, 2)
    ...
    def test_set_balance(self, mock_getkey):
        ...
        self.assertEquals(2, c.ss_balance_left)
        self.assertEquals(1, c.ns_balance_left)
...
class TestBasic(unittest.TestCase):
    ...
    def test_set_outpath(self, mock_getkey):
        ...
        self.assertRaises(ValueError, hcs.set_outpath, 'dummy',
                          [c1, c2, diff])
    ...
    def test_crop_and_scale(self, mock_getkey, mock_crop, moc_enlarge):
        ...
        self.assertEquals(p, new_c[0].ls_path)
    ...
    def test_get_normalization(self, mock_getkey):
        ...
        c5.correction = 5
        c6.correction = 6
        self.assertEqual(5, hcs.get_normalization([c1, c2, c3, c4, c5, c6],
                                                  [1, 2, 3, 4, 5, 6],
                                                  -1,
                                                  [5, 11, 13]))
        self.assertEqual(3, hcs.get_normalization([c2, c3, c4],
                                                  [2, 3, 4],
                                                  -2,
                                                  [5, 11, 13]))
...
class TestMock(unittest.TestCase):
    ...
    def setUp(self):
       ...
       1       2           4000       0.151859       0.152177     0.00559696       0.127681       0.172393'''
        hiconf = {'HiccdStitch_Bin1_Skip_Top_Lines': 1,
    ...
    def test_CubeNormStep(self, mock_crop, mock_cubenorm, mock_getkey, mock_PathSetunlink):
        ...
        with patch('PyRISE.HiccdStitch.open', m):
            ...
            self.assertEquals(Path('dummy/PSP_010502_2090_RED5_0.cubenorm.cub'),
                              new_c.nextpath)
    ...
    def test_make_balance(self, mock_getkey, mock_algebra):
        c = hcs.HiccdStitchCube('dummy/PSP_010502_2090_RED5_0')
        c.correction = 1
        hcs.make_balance(c, self.conf['HiccdStitch'], Path('dummy_balance.cub'))
        mock_algebra.assert_has_calls([call(Path('dummy/PSP_010502_2090_RED5_0'),
                                            a=1, c=0, operator='unary',
    ...
    def test_BalanceStep(self, m_getkey, m_mask, m_algebra, m_crop, m_enlarge,
        ...
        cubes = hcs.BalanceStep([c1, c2, c3, c4, c5, c6], self.conf['HiccdStitch'])
        self.assertEqual(1, cubes[0].lstats)
        self.assertEqual(1, cubes[0].correction)
        self.assertEqual(Path('dummy/PSP_010502_2090_RED1.balance.cub'), cubes[0].nextpath)
    ...
    def test_SpecialProcessingFlags(self, m_getkey, m_editlab):
        ...
        m_editlab.assert_has_calls([call(p, grpname='Instrument',
                                         ...
                                         option='MODKEY', value='NOMINAL')])
        m_editlab.reset_mock()
        c.hical_status = 'BadCal'
        hcs.SpecialProcessingFlags(c)
        m_editlab.assert_has_calls([call(p, grpname='Instrument',
                                         keyword='Special_Processing_Flag',
                                         option='MODKEY', value='BADCAL')])
        m_editlab.reset_mock()
        c.cubenormstep = True
        hcs.SpecialProcessingFlags(c)
        m_editlab.assert_has_calls([call(p, grpname='Instrument',

...
# and on the Perl HiSlither program: ($Revision: 1.40 $ $Date: 2011/01/31 20:12:36 $)
# by Guy McArthur
...
def main():
    ...
    if cube_check(red4, ir10, bg12):
        HiSlither(red4, ir10, bg12, keep=args.keep)

    if cube_check(red5, ir11, bg13):
        HiSlither(red5, ir11, bg13, keep=args.keep)
...
def HiSlither(red: hicolor.HiColorCube, ir: hicolor.HiColorCube,
    ...
    if ir is None:
        # If the IR band is missing, it is filled with a "dummy" band consisting
        # entirely of null pixels. This band is created by masking the red band with
        # itself. The dummy band allows for RGB (synthetic B) to be created by HiBeautify
        # even if the IR band is missing.
